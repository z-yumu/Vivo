...
{     
  ...
  //订单明细的输出
    public function paylist()
    {
      ...
      $list= $pay->getpay($merchantinfoid);
  ...
  //结算中的列表的输出
    public function bill()
    {
      $bill= new PayModel;
      $merchantinfoid=session('merchantinfoid','','merchant');
      $list= $bill->getbilllist($merchantinfoid);
      $sum= $bill->where([
    ...
    public function billdetails()
    {
      $bill= new PayModel;
      $merchantinfoid=session('merchantinfoid','','merchant');
      $list= $bill->getbilldetails($merchantinfoid);
      $this->assign('list',$list);
      return $this->fetch();  
    }
     //已结算表的输出
    public function billsuccesslist()
    {
      $bills= new billModel;
      $merchantinfoid=session('merchantinfoid','','merchant');
      $list=$bills->getbillsuccesslist($merchantinfoid);
       $this->assign('list',$list);
      return $this->fetch();   
    }
     //已结算表的详单输出
    public function billsuccesslistdetails($billnumber)
    {
      $bill= new PayModel;
      $list= $bill->getbillsuccesslistdetails($billnumber);
      $this->assign('list',$list);
      return $this->fetch();   
    }
     //商家账号
    public function bank()
    {
      ...
      $merchantinfoid=session('merchantinfoid','','merchant');
      $bank= $merchantsettlement->where('infoid',$merchantinfoid)->find();
